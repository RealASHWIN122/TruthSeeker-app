...
def get_result(input_path, advanced_folder_path, checkpoint_path):
    ...
    try:
        advanced_video_folder_path = os.path.join(
            advanced_folder_path, input_file_name.split('.')[0]
        )
        ...
        for idx in range(len(combined_frames)):
            cv2.imwrite(
                os.path.join(advanced_video_folder_path, f'lip{idx}.png'),
                combined_frames[idx]
            )
            if idx < len(residue_frames):
                cv2.imwrite(
                    os.path.join(advanced_video_folder_path, f'res{idx}.png'),
                    residue_frames[idx]
                )

        combined_frames = combined_frames.reshape((1,) + combined_frames.shape)
        residue_frames = residue_frames.reshape((1,) + residue_frames.shape)

        print(
            f"Shape of Color Frames: {combined_frames.shape} | "
            f"Structure Frames: {residue_frames.shape}"
        )

        model = LIPINC_V2()
        model.load_weights(checkpoint_path)

        preds = model.predict([combined_frames, residue_frames])
        real_prob = round(float(preds[0][1]), 3)
